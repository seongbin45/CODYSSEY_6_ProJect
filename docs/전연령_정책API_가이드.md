...
- 목록은 요청마다 1페이지를 치지 않습니다. 군산 대시보드와 같이 `pageSize=100`으로 끝까지 받아
  ...
  갱신: GitHub Actions `sync-youth`가 매일 03:00·15:00 KST에 `scripts/sync_youth.py`를 돌리고
  스냅샷이 바뀌면 커밋합니다. 수동은 `py -3 scripts/sync_youth.py`, `/sync-youth`,
  또는 `/api/policies?sync=1`(로컬·배치용).
